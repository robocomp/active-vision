	...
	switch( state )
 	{
		...
		case State::SENSE:
			...
			newPose = correctPose + getInitialSample();
...
{
	QVec res(6); 
	res.inject(QVec::uniformVector(3, -700, 700),0);
	res.inject(QVec::uniformVector(1, -0.3, 0.3),3);	
	res.inject(QVec::uniformVector(1, -0.8, 0.8),4);
	res.inject(QVec::uniformVector(1, -0.3, 0.3),5);	
	res[1]= 0; 
	return res;
}
